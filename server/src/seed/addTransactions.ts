...
function seedTransactions() {
  ...
  // u103 additional: Apple x5 (in_store, cash)
  {
    ...
    const { total, resolved } = totalFor(shopId, items);
    addTransaction.run(shopId, "p103", total, "Store #1", "cash", "in_store");
    const { id } = getLastInsertRowId.get() as { id: number };
    for (const r of resolved) addTransactionItem.run(id, r.itemId, r.qty);
    linkUserTransaction.run("u103", id);
  }
  ...
  // u104 additional: Pen x1 (card)
  {
    ...
    const { total, resolved } = totalFor(shopId, items);
    addTransaction.run(shopId, "p104", total, "Kiosk B", "card", "in_store");
    const { id } = getLastInsertRowId.get() as { id: number };
    for (const r of resolved) addTransactionItem.run(id, r.itemId, r.qty);
    linkUserTransaction.run("u104", id);
  }
  ...
  // u105 additional: T-Shirt x2 (in_store, cash)
  {
    ...
    const { total, resolved } = totalFor(shopId, items);
    addTransaction.run(shopId, "p105", total, "Outlet C", "cash", "in_store");
    const { id } = getLastInsertRowId.get() as { id: number };
    for (const r of resolved) addTransactionItem.run(id, r.itemId, r.qty);
    linkUserTransaction.run("u105", id);
  }
